...
// Get user's recent trips for quick rebook (optional)
...
// ==================== BOOKING FUNCTION - UPDATED VERSION ====================
async function bookRide(event) {
    ...
    const apiUrls = [
        '../api/create_trip.php',                    // Relative path (most reliable)
        './api/create_trip.php',                      // Another relative path
        '/fleetflow/api/create_trip.php',             // Absolute path with project name
        window.location.origin + '/fleetflow/api/create_trip.php', // Full URL with origin
        window.location.origin + '/api/create_trip.php'             // Full URL without project
    ];
    ...
    for (const apiUrl of apiUrls) {
        try {
            ...
            if (response.ok) {
                ...
                console.log(`Failed response from ${apiUrl}:`, errorText.substring(0, 200));
    ...
    showNotification('Network error: Could not connect to server. Please check your connection and try again.', 'error');
    document.getElementById('loadingOverlay').style.display = 'none';
    document.getElementById('bookBtn').disabled = false;
...
async function testAPIConnection() {
    ...
    const testUrls = [
        '../api/create_trip.php',
        './api/create_trip.php',
        '/fleetflow/api/create_trip.php',
        window.location.origin + '/fleetflow/api/create_trip.php',
        window.location.origin + '/api/create_trip.php'
    ];
    ...
    for (const url of testUrls) {
        try {
            ...
            console.log(`✅ URL ${url} - Status: ${status}`, data || text.substring(0, 100));
        } catch (error) {
            results.push({
                url,
                error: error.message
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-run API test after 2 seconds (for debugging)
